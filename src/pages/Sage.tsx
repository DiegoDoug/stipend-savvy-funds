...
const Sage: React.FC = () => {
  ...
  const handleCreateExpenseFromAI = useCallback(async (expense: {
    description: string;
    amount: number;
    category: string;
    date: string;
  ...
  const handleCreateIncomeFromAI = useCallback(async (income: {
    description: string;
    amount: number;
    category: string;
    date: string;
  ...
  const handleEditGoalFromAI = useCallback(async (id: string, data: {
    name?: string;
    ...
    const {
      error
    } = await supabase.from('savings_goals').update(updateData).eq('id', id).eq('user_id', user.id);
    if (error) {
      toast({
        title: 'Error',
  ...
  const handleEditExpenseFromAI = useCallback(async (id: string, data: {
    description?: string;
    amount?: number;
    category?: string;
    date?: string;
    ...
    const {
      error
    } = await supabase.from('transactions').update(updateData).eq('id', id).eq('user_id', user.id);
    if (error) {
      toast({
        title: 'Error',
  ...
  const handleEditIncomeFromAI = useCallback(async (id: string, data: {
    description?: string;
    amount?: number;
    category?: string;
    date?: string;
  ...
  const handleCreateBudgetFromAI = useCallback(async (budget: {
    ...
    const success = await createBudget(budget.name, budget.expenseAllocation, budget.savingsAllocation, budget.description, linkedGoalId);
    if (success) {
  ...
  return <>
      {/* Welcome Onboarding */}
      {showOnboarding && <SageWelcome onComplete={completeOnboarding} onSkip={completeOnboarding} />}
      ...
      <div className="container mx-auto p-4 lg:p-6 h-[calc(100vh-8rem)] flex flex-col">
        ...
        <Tabs defaultValue="chat" className="flex-1 flex flex-col min-h-0">
          ...
          <TabsContent value="chat" className="flex-1 min-h-0 mt-0">
            <FinancialAdvisorChat financialContext={financialContext} standalone={true} onCreateGoal={handleCreateSuggestedGoal} onCreateExpense={handleCreateExpenseFromAI} onCreateIncome={handleCreateIncomeFromAI} onEditGoal={handleEditGoalFromAI} onEditExpense={handleEditExpenseFromAI} onEditIncome={handleEditIncomeFromAI} onAddFundsToGoal={handleAddFundsToGoal} onCreateBudget={handleCreateBudgetFromAI} onEditBudget={handleEditBudgetFromAI} onDeleteBudget={handleDeleteBudgetFromAI} onLinkGoalToBudget={handleLinkGoalToBudgetFromAI} />
          </TabsContent>
