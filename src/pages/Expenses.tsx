...
export default function Expenses() {
  ...
  const filteredExpenses = expenses.filter(expense => expense.description.toLowerCase().includes(searchTerm.toLowerCase()) && (selectedCategory === null || expense.category === selectedCategory)).sort((a, b) => {
  ...
  const largestCategory = Object.entries(categoryTotals).reduce((max, [category, amount]) => amount > max.amount ? {
    category,
    amount
  } : max, {
    category: '',
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard title="Total Spent (Month)" value={`$${totalExpenses.toLocaleString()}`} change="-12% vs last month" changeType="positive" icon={<CreditCard size={24} />} />
        <StatCard title="Daily Average" value={`$${avgDaily.toFixed(2)}`} subtitle="Based on 30 days" icon={<Calendar size={24} />} />
        <StatCard title="Largest Category" value={largestCategory.category ? categoryLabels[largestCategory.category as keyof typeof categoryLabels] || largestCategory.category : "None"} subtitle={`$${largestCategory.amount.toFixed(2)}`} icon={<Filter size={24} />} />
      </div>
      ...
      <div className="budget-card">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <button onClick={() => setSelectedCategory(null)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${selectedCategory === null ? 'bg-primary text-primary-foreground' : 'bg-accent/50 hover:bg-accent'}`}>
              All Categories
            </button>
            {availableCategories.map(category => <button key={category} onClick={() => setSelectedCategory(category)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${selectedCategory === category ? 'bg-primary text-primary-foreground' : 'bg-accent/50 hover:bg-accent'}`}>
        ...
        <div className="flex gap-2 mt-4">
          ...
          {['date', 'amount'].map(sort => <button key={sort} onClick={() => setSortBy(sort)} className={`px-2 py-1 text-xs rounded ${sortBy === sort ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      <div className="budget-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(categoryTotals).map(([category, amount]) => <div key={category} className="p-4 bg-accent/20 rounded-lg border border-border/50">
              ...
              <div className="mt-2">
                <div className="w-full bg-muted rounded-full h-1.5">
                  <div className={`h-full rounded-full bg-category-${category}`} style={{
      ...
      <div className="budget-card">
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {filteredExpenses.map(expense => <div key={expense.id} className="expense-item">
